...
import net.sourceforge.htmlunit.xpath.xml.utils.URI.MalformedURIException;
...
public class SystemIDResolver {
   ...
   *
   * <p>The relative URI is a local filesystem path. The path can be absolute or relative. If it is
   * a relative path, it is resolved relative to the system property "user.dir" if it is available;
   * if not (i.e. in an Applet perhaps which throws SecurityException) then we just return the
   * relative path. The space and backslash characters are also replaced to generate a good absolute
   * URI.
   *
  ...
  public static boolean isAbsoluteURI(String systemId) {
     ...
     * contains a colon character cannot be used as the first segment of a relative URI path (e.g.,
