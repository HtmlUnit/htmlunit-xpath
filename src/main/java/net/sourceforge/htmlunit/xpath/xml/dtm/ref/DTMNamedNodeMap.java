...
public class DTMNamedNodeMap implements NamedNodeMap {
   ...
   * As the <code>nodeName</code> attribute is used to derive the name which the node must be stored
   ...
   *     INUSE_ATTRIBUTE_ERR: Raised if <code>arg</code> is an <code>Attr</code> that is already an
   *     attribute of another <code>Element</code> object. The DOM user must explicitly clone <code>
   *     Attr</code> nodes to re-use them in other elements.
   */
  @Override
  ...
  /**
   * Adds a node using its <code>namespaceURI</code> and <code>localName</code>. If a node with that
   * namespace URI and that local name is already present in this map, it is replaced by the new
   * one. <br>
   ...
   * have a default value when this map contains the attributes attached to an element, as returned
   * by the attributes attribute of the <code>Node</code> interface. If so, an attribute immediately
   * appears containing the default value as well as the corresponding namespace URI, local name,
   * and prefix when applicable. <br>
   * HTML-only DOM implementations do not need to implement this method.
   *
