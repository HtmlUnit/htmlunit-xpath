...
public class URI implements Serializable {
   ...
   * component is validated for syntax and some basic semantic checks are performed as well. See the
   * individual setter methods for specifics.
   *
   * @param p_scheme the URI scheme (cannot be null or empty)
  ...
  private void initializeScheme(String p_uriSpec) throws MalformedURIException {
    ...
    if (scheme.length() == 0) {
      throw new MalformedURIException(
          XMLMessages.createXMLMessage(
              XMLErrorResources.ER_NO_SCHEME_INURI, null)); // "No scheme found in URI.");
    } else {
      setScheme(scheme);
    }
  }
  ...
  /**
   * Set the host for this URI. If null is passed in, the userinfo field is also set to null and the
   * port is set to -1.
   *
  ...
  /**
   * Determine whether a string is syntactically capable of representing a valid IPv4 address or the
   * domain name of a network host. A valid IPv4 address consists of four decimal digit groups
   * separated by a '.'. A hostname consists of domain labels (each of which must begin and end with
   * an alphanumeric but may contain '-') separated & by a '.'. See RFC 2396 Section 3.2.2.
   *
