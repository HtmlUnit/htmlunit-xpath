...
public interface DTMFilter {
   ...
   * Normally, this function will be called by the implementation of <code>DTMIterator</code>; it is
   * not normally called directly from user code.
   *
   ...
   * localnames than namespaces, which is fairly likely), this is going to break. It might be safer
   * to encapsulate the details with a makeExpandedName method and make that responsible for setting
   * up the wildcard version as well.
   *
   * @param nodeHandle int Handle of the node.
   * @param whatToShow one of SHOW_XXX values.
