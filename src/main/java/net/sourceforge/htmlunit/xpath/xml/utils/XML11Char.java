...
public class XML11Char {
   ...
   *
   * <p>If the program chooses to apply the mask directly to the <code>XML11CHARS</code> array, then
  ...
  public static boolean isXML11ValidLiteral(int c) {
    return c < 0x10000
        && ((XML11CHARS[c] & MASK_XML11_VALID) != 0 && (XML11CHARS[c] & MASK_XML11_CONTROL) == 0
            || (0x10000 <= c && c <= 0x10FFFF));
  ...
  /**
   * Returns true if the specified character is a valid name character as defined by production [4a]
    ...
    while (i < length) {
      ...
      if (!isXML11Name(ch)) {
        if (++i < length && isXML11NameHighSurrogate(ch)) {
          ...
          if (!isLowSurrogate(ch2) || !isXML11Name(supplemental(ch, ch2))) {
            return false;
          }
        } else {
          return false;
  ...
  public static boolean isXML11ValidNCName(String ncName) {
    ...
    if (!isXML11NCNameStart(ch)) {
      if (length > 1 && isXML11NameHighSurrogate(ch)) {
        ...
        if (!isLowSurrogate(ch2) || !isXML11NCNameStart(supplemental(ch, ch2))) {
          return false;
        }
        i = 2;
      } else {
    ...
    while (i < length) {
      ...
      if (!isXML11NCName(ch)) {
        if (++i < length && isXML11NameHighSurrogate(ch)) {
          ...
          if (!isLowSurrogate(ch2) || !isXML11NCName(supplemental(ch, ch2))) {
            return false;
          }
        } else {
          return false;
