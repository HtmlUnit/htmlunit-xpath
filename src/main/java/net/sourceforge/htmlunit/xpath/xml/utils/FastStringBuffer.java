...
public class FastStringBuffer {
   ...
   * called for the first chunk of a multi-chunk output, or one following unsuppressed whitespace in
   * a previous chunk.
   *
   ...
   * within a single chunk before flowing over to the next chunk. For example, if m_chunkbits is set
   * to 15, each chunk can contain up to 2^15 (32K) characters
   */
  int m_chunkBits = 15;

  /**
   * Field m_maxChunkBits affects our chunk-growth strategy, by saying what the largest permissible
   ...
   * allocated at one size before we encapsulate them into the first chunk of the next size up. For
   * example, if m_rechunkBits is set to 3, then after 8 chunks at a given size we will rebundle
   * them as the first element of a FastStringBuffer using a chunk size 8 times larger (chunkBits
  ...
  /**
   * Field m_chunkMask is m_chunkSize-1 -- in other words, m_chunkBits worth of low-order '1' bits,
   * useful for shift-and-mask addressing within the chunks.
   */
  int m_chunkMask; // =m_chunkSize-1;

  /**
   * Field m_array holds the string buffer's text contents, using an array-of-arrays. Note that this
   * array, and the arrays it contains, may be reallocated when necessary in order to allow the
   * buffer to grow; references to them should be considered to be invalidated after any append.
   * However, the only time these arrays are directly exposed is in the sendSAXcharacters call.
   */
  char[][] m_array;
   ...
   *
   * <p>The insertion point for append operations is addressed by the combination of m_lastChunk and
   * m_firstFree.
   */
  int m_lastChunk = 0;
   ...
   *
   * <p>ISSUE: Should this call assert initial size, or fixed size? Now configured as initial, with
   * a default for fixed.
   *
   ...
   * storage does exist, its contents are unpredictable. The only safe use for our setLength() is to
   ...
   * hence a double copy is needed. By using ensureCapacity we hope to minimize the heap overhead of
   ...
   *
   * <p>NOTE THAT after calling append(), previously obtained references to m_array[] may no longer
   * be valid.
   *
  ...
  /**
   * Append part of the contents of a Character Array onto the FastStringBuffer, growing the storage
   * if necessary.
   *
   * <p>NOTE THAT after calling append(), previously obtained references to m_array[] may no longer
   * be valid.
  ...
  /**
   * Append the contents of another FastStringBuffer onto this FastStringBuffer, growing the storage
   * if necessary.
   *
   * <p>NOTE THAT after calling append(), previously obtained references to m_array[] may no longer
   * be valid.
  ...
  public final void append(FastStringBuffer value) {
    ...
    while (strlen > 0) {
      ...
      System.arraycopy(
          value.m_array[sourcechunk], sourcecolumn, m_array[m_lastChunk], m_firstFree, runlength);
  ...
  public boolean isWhitespace(int start, int length) {
    ...
    while (length > 0) {
      ...
      if (sourcechunk == 0 && m_innerFSB != null)
        ...
        chunkOK =
            net.sourceforge.htmlunit.xpath.xml.utils.XMLCharacterRecognizer.isWhiteSpace(
   ...
   * edited; it's the user's responsibility to manage access to the FastStringBuffer to prevent this
   * problem from arising.
   *
   * <p>Note too that there is no promise that the output will be sent as a single call. As is
   * always true in SAX, one logical string may be split across multiple blocks of memory and hence
   * delivered as several successive events.
   *
  ...
  /**
   * Sends the specified range of characters as one or more SAX characters() events, normalizing the
   * characters according to XSLT rules.
   *
   * @param ch SAX ContentHandler object to receive the event.
   * @param start Offset of first character in the range.
   * @param length Number of characters to send.
   ...
   *           following chunk (if any) should be completely suppressed.
   *     </dl>
   *
   * @exception org.xml.sax.SAXException may be thrown by handler's characters() method.
   */
  public int sendNormalizedSAXcharacters(org.xml.sax.ContentHandler ch, int start, int length)
    ...
    for (int i = startChunk; i < stopChunk; ++i) {
      if (i == 0 && m_innerFSB != null)
        stateForNextChunk =
            m_innerFSB.sendNormalizedSAXcharacters(ch, startColumn, m_chunkSize - startColumn);
      else
        stateForNextChunk =
            sendNormalizedSAXcharacters(
                m_array[i], startColumn, m_chunkSize - startColumn, ch, stateForNextChunk);
   ...
   *       <dd>When false, causes trailing whitespace to be converted to a single space; when true,
   *           causes it to be discarded entirely. Should be set TRUE for the last or only chunk.
   *     </dl>
   *
  ...
  static int sendNormalizedSAXcharacters(
      char ch[], int start, int length, org.xml.sax.ContentHandler handler, int edgeTreatmentFlags)
      throws org.xml.sax.SAXException {
  ...
  /**
   * Encapsulation c'tor. After this is called, the source FastStringBuffer will be reset to use the
   * new object as its m_innerFSB, and will have had its chunk size reset appropriately. IT SHOULD
   * NEVER BE CALLED EXCEPT WHEN source.length()==1<<(source.m_chunkBits+source.m_rebundleBits)
   *
