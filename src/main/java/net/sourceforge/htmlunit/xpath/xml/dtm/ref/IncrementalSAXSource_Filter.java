...
import net.sourceforge.htmlunit.xpath.xml.utils.ThreadControllerWrapper;
...
public class IncrementalSAXSource_Filter
  ...
  public void ignorableWhitespace(char[] ch, int start, int length)
    ...
    if (clientContentHandler != null) clientContentHandler.ignorableWhitespace(ch, start, length);
  }
  ...
  /**
   * In the SAX delegation code, I've inlined the count-down in the hope of encouraging compilers to
   * deliver better performance. However, if we subclass (eg to directly connect the output to a DTM
   * builder), that would require calling super in order to run that logic... which seems inelegant.
   * Hence this routine for the convenience of subclasses: every [frequency] invocations, issue a
   * co_yield.
   *
  ...
  /**
   * co_entry_pause is called in startDocument() before anything else happens. It causes the filter
   * to wait for a "go ahead" request from the controller before delivering any events. Note that
   * the very first thing the controller tells us may be "I don't need events after all"!
   */
   ...
   * startReader() method), we can do so very quickly by throwing a reserved exception to it. If the
   * stream is coming from another source, we can't do that because its caller may not be prepared
   * for this "normal abnormal exit", and instead we put ourselves in a "spin" mode where events are
   * discarded.
   */
  ...
  /**
   * Launch a thread that will run an XMLReader's parse() operation within a thread, feeding events
   * to this IncrementalSAXSource_Filter. Mostly a convenience routine, but has the advantage that
   * -- since we invoked parse() -- we can halt parsing quickly via a StopException rather than
  ...
  public void run() {
    ...
    try {
      ...
      if (inner instanceof StopException) {
        ...
        if (DEBUG) {
          System.out.println(
              "Active IncrementalSAXSource_Filter UNEXPECTED SAX exception: " + inner);
   ...
   *     _could_ actually throw the exception, but that would require runinng deliverMoreNodes() in
   *     a try/catch... and for many applications, exception will be simply be treated as "not TRUE"
   *     in any case.
   */
  @Override
    ...
    try {
      Object result =
          fCoroutineManager.co_resume(
              parsemore ? Boolean.TRUE : Boolean.FALSE, fControllerCoroutineID, fSourceCoroutineID);
